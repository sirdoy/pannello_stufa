...
must_haves:
  truths:
    - "Token stored in IndexedDB survives browser restart"
    - "Token stored in localStorage survives browser restart"
    - "Token can be loaded from either storage on startup"
    - "Persistent storage is requested to prevent eviction"
  artifacts:
    - path: "lib/tokenStorage.js"
      provides: "Dual persistence storage layer"
      exports: ["saveToken", "loadToken", "clearToken", "checkPersistence"]
      min_lines: 80
  key_links:
...
<objective>
Create dual persistence storage layer for FCM tokens using IndexedDB (primary) and localStorage (fallback).

Purpose: Fix the critical bug where FCM tokens don't survive browser restarts. IndexedDB provides robust storage, localStorage provides fallback for edge cases. Request persistent storage to prevent browser eviction.

Output: `lib/tokenStorage.js` module with save/load/clear functions that write to both storage mechanisms.
</objective>
...
<task type="auto">
  <name>Task 1: Install Dexie.js dependency</name>
  <files>package.json</files>
  <action>
Install dexie package for IndexedDB wrapper:
```bash
npm install dexie
```
Dexie.js v4.2.1+ provides:
- Promise-based IndexedDB API
- Automatic versioning and migration
- Works in service workers
- Handles browser bugs (especially Safari)

Do NOT install ua-parser-js yet - that's in Plan 02.
  </action>
  <verify>
Run `npm ls dexie` to confirm installation. Package.json should show dexie in dependencies.
  </verify>
  <done>
dexie appears in package.json dependencies with version ^4.x
  </done>
</task>

<task type="auto">
  <name>Task 2: Create tokenStorage.js module</name>
  <files>lib/tokenStorage.js</files>
  <action>
Create new file `lib/tokenStorage.js` implementing dual persistence:
...
- Uses Dexie for clean IndexedDB API
- Dual write on save (IndexedDB + localStorage)
- Prioritizes IndexedDB on load, falls back to localStorage
- Auto-syncs from localStorage to IndexedDB if IDB was cleared
- Requests `navigator.storage.persist()` on first save
- Includes health check function for debugging
  </action>
  <verify>
1. File exists at lib/tokenStorage.js
2. Exports: saveToken, loadToken, clearToken, checkPersistence, requestPersistentStorage, updateLastUsed, getTokenAge, getStorageStatus
3. No syntax errors: `node -c lib/tokenStorage.js` (may fail due to imports, that's OK)
4. Run `npm test` - should pass (no breaking changes to existing code)
  </verify>
  <done>
lib/tokenStorage.js exists with all required exports. Module provides dual persistence to IndexedDB and localStorage.
  </done>
</task>
...
<verification>
1. `npm ls dexie` shows dexie installed
2. `lib/tokenStorage.js` file exists with all exports
3. `npm test` passes (no regressions)
</verification>

<success_criteria>
- dexie package installed in package.json
- lib/tokenStorage.js module created with dual persistence logic
- No breaking changes to existing notification code
</success_criteria>

<output>
After completion, create `.planning/phases/01-token-lifecycle-foundation/01-01-SUMMARY.md`
</output>
