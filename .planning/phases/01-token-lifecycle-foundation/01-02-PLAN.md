...
must_haves:
  truths:
    - "Device can be uniquely identified by browser+OS combination"
    - "Same device re-registering produces same deviceId"
    - "Different devices produce different deviceIds"
    - "Device name is human-readable (e.g., 'Chrome on Windows')"
  artifacts:
    - path: "lib/deviceFingerprint.js"
      provides: "Device identification from user agent"
      exports: ["generateDeviceFingerprint", "parseUserAgent"]
      min_lines: 50
  key_links:
    - from: "lib/deviceFingerprint.js"
      to: "ua-parser-js"
      via: "import UAParser from 'ua-parser-js'"
      pattern: "import.*UAParser.*from.*ua-parser-js"
---

<objective>
Create device fingerprinting module to uniquely identify devices for multi-device support.

Purpose: Enable TOKEN-05 (multi-device) and TOKEN-06 (device fingerprinting). Generate stable device IDs from user agent so same browser/OS produces same ID, preventing duplicate token accumulation.

Output: `lib/deviceFingerprint.js` module with fingerprint generation and UA parsing functions.
</objective>
...
<task type="auto">
  <name>Task 1: Install ua-parser-js dependency</name>
  <files>package.json</files>
  <action>
Install ua-parser-js package for user agent parsing:
```bash
npm install ua-parser-js
```
ua-parser-js v1.0.38+ provides:
- Database of 8,000+ user agent patterns
- Handles edge cases (Chromium variants, mobile browsers)
- Extracts browser, OS, device type
- Regularly updated for new browsers
  </action>
  <verify>
Run `npm ls ua-parser-js` to confirm installation. Package.json should show ua-parser-js in dependencies.
  </verify>
  <done>
ua-parser-js appears in package.json dependencies with version ^1.x
  </done>
</task>

<task type="auto">
  <name>Task 2: Create deviceFingerprint.js module</name>
  <files>lib/deviceFingerprint.js</files>
  <action>
Create new file `lib/deviceFingerprint.js` implementing device identification:
...
- Uses ua-parser-js for robust UA parsing
- Device ID based on browser NAME + OS NAME only (not versions) for stability
- Simple hash function avoids crypto dependency
- Screen size and timezone captured for debugging
- Human-readable display names like "Chrome on Windows"
- Helper functions for device comparison and formatting
  </action>
  <verify>
1. File exists at lib/deviceFingerprint.js
2. Exports: generateDeviceFingerprint, parseUserAgent, getCurrentDeviceFingerprint, isSameDevice, formatDeviceInfo
3. No syntax errors: `node -c lib/deviceFingerprint.js` (may fail due to imports, OK)
4. Run `npm test` - should pass (no breaking changes)
  </verify>
  <done>
lib/deviceFingerprint.js exists with all required exports. Module provides stable device identification from user agent.
  </done>
</task>
...
<verification>
1. `npm ls ua-parser-js` shows package installed
2. `lib/deviceFingerprint.js` file exists with all exports
3. `npm test` passes (no regressions)
</verification>

<success_criteria>
- ua-parser-js package installed in package.json
- lib/deviceFingerprint.js module created with fingerprinting logic
- Device ID is stable (same browser/OS = same ID regardless of version)
- No breaking changes to existing code
</success_criteria>

<output>
After completion, create `.planning/phases/01-token-lifecycle-foundation/01-02-SUMMARY.md`
</output>
