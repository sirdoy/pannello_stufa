...
1. **Add mode switching handlers** (near line 549, after handleClearSemiManual):
   ```jsx
   const handleSetManualMode = async () => {
     // Disable scheduler = manual mode
     await fetch('/api/scheduler/mode', {
       method: 'POST',
   ...
   const handleSetAutomaticMode = async () => {
     // Enable scheduler, clear semi-manual = automatic mode
     await fetch('/api/scheduler/mode', {
       method: 'POST',
       headers: { 'Content-Type': 'application/json' },
       body: JSON.stringify({ enabled: true, semiManual: false })
     });
...
- Mode handlers wired up for Manuale and Automatica
  </done>
</task>
