...
2. **Create simplified status helper** (place near line 572, before or replacing getStatusInfo):
   ...
   const getStatusDisplay = (status) => {
     ...
     // 🌡️ MODULATION - Modulazione (Ocean)
...
1. **Locate the header section** (around line 832-835):
   ```jsx
...
2. **Add status Badge and HealthIndicator** after the Heading:
   ```jsx
   {/* Header - Ember Noir style */}
   <div className="flex items-center justify-between gap-3 mb-6">
     ...
     <div className="flex items-center gap-2">
       <Badge
         variant={getStatusDisplay(status).variant}
         pulse={getStatusDisplay(status).pulse}
         size="sm"
       >
         {getStatusDisplay(status).label}
       </Badge>
       <HealthIndicator
         status={getStatusDisplay(status).health}
         size="sm"
         showIcon={true}
         label=""
       />
...
3. **Store status display result to avoid repeated calls**:
   Near line 734 (after `const statusInfo = getStatusInfo(status);`), add:
   ```jsx
   const statusDisplay = getStatusDisplay(status);
   ```

   Then update the header to use `statusDisplay`:
   ```jsx
   <Badge
...
3. `npm run dev` - Check visual: header should show compact Badge + HealthIndicator alongside "Stufa" title
  </verify>
  <done>
