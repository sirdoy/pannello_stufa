...
must_haves:
  truths:
    - "API forecast response includes weatherCode for each day"
  ...
  artifacts:
    - path: "app/api/weather/forecast/route.js"
      provides: "weatherCode field in forecast array"
      contains: "weatherCode: data.daily.weather_code"
  ...
  key_links:
    ...
    - from: "app/components/weather/ForecastDayCard.jsx"
      to: "app/api/weather/forecast/route.js"
      via: "weatherCode field for WeatherIcon"
      pattern: "day\\.weatherCode"
---

<objective>
Create the foundational WeatherIcon component, helper utilities, and update the API to provide weatherCode.

Purpose: These utilities are used by all weather components (CurrentConditions, ForecastRow, ForecastDaySheet). The API update ensures forecast data includes the WMO weather code needed by WeatherIcon.

Output:
- Updated API route with weatherCode in forecast array
...
# Current API route (needs weatherCode added)
@app/api/weather/forecast/route.js
</context>
...
<task type="auto">
  <name>Task 1: Update API route to include weatherCode in forecast</name>
  <files>app/api/weather/forecast/route.js</files>
  <action>
Update the weather forecast API route to include the raw WMO weather code in each forecast day object.
...
**Why this is needed:**
- ForecastDayCard and ForecastDaySheet use WeatherIcon component
- WeatherIcon requires the raw WMO code (0-99) to select the correct Lucide icon
- The `condition` object only has `{ description, icon }` which uses standard codes like '01', '09'
- Adding weatherCode allows direct mapping to Lucide components

**Update API documentation comment (line 18-24) to reflect new field:**
```javascript
* Response:
* {
*   success: true,
...
```
  ...
  <verify>
- API route includes weatherCode in dailyForecast mapping
- API documentation comment updated
- No other changes to the route logic
  </verify>
  <done>
API forecast response includes weatherCode field for each forecast day
  </done>
</task>

<task type="auto">
  <name>Task 2: Create WeatherIcon component</name>
...
<task type="auto">
  <name>Task 3: Create weatherHelpers utilities</name>
...
2. `getTemperatureComparison(outdoorTemp, indoorTemp)`:
   ...
   - Note: "piu" not "piu" to avoid encoding issues
...
1. API route returns weatherCode in forecast array
2. Both UI files exist in app/components/weather/
3. WeatherIcon can be imported and renders icons
4. weatherHelpers functions return expected formats
5. No TypeScript/ESLint errors
</verification>

<success_criteria>
- API forecast includes weatherCode field (WMO code 0-99)
