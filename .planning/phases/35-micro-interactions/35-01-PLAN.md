...
must_haves:
  truths:
    - "Animation tokens are centralized in @theme block"
    - "Stagger system exists for list/grid animations"
    - "Reduced motion preserves functional animations (spinners, progress)"
    - "Three distinct ease curves available (enter, exit, spring)"
  artifacts:
...
<task type="auto">
  <name>Task 3: Refine reduced motion support</name>
  <files>app/globals.css</files>
  <action>
Replace the current reduced motion block (lines 1054-1062) with refined rules that distinguish decorative vs functional animations:

```css
/* Reduced motion support - refined for accessibility */
@media (prefers-reduced-motion: reduce) {
  ...
  /* Functional animations - keep but slow down */
  .animate-spin,
  .animate-shimmer,
  ...
  /* Shake animation - keep for error feedback but reduce intensity */
  .animate-shake {
    animation-duration: 0.3s !important;
  }

  /* General transition reduction - but not complete removal */
  *:not(.animate-spin):not(.animate-shimmer):not(.animate-pulse-ember):not(.animate-progress-indeterminate),
  *::before,
  *::after {
...
This replaces the old blanket rule with nuanced handling:
- Decorative: disabled completely
- Functional (spinners, progress, shimmer): slowed but preserved
- Transitions: reduced to 50ms (fast state changes, not instant)
- Page transitions: disabled
  </action>
  <verify>
Grep globals.css for "Functional animations" - comment should exist indicating refined approach.
Grep for ".animate-spin" in the reduced motion block - should NOT have "animation: none".
  </verify>
  <done>
Reduced motion rules distinguish decorative (disabled) from functional (preserved) animations. Spinners, progress indicators, and shimmer effects remain animated for users with vestibular disorders.
  </done>
</task>
...
3. Reduced motion preserves functional animations (spinners, progress)
...
- Reduced motion media query handles functional animations differently from decorative
