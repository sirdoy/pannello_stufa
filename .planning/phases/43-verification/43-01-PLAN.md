...
**Step 5:** Update or replace `types/external-apis.d.ts` to re-export from the new directory structure.

**IMPORTANT — Type wiring approach:** Use EXPLICIT EXPORTS (not ambient/global declarations). Each file (hue.d.ts, netatmo.d.ts, camera.d.ts) should export its interfaces using `export interface`. The barrel file `types/external-apis.d.ts` (or `types/external-apis/index.ts`) should re-export all types: `export * from './hue'; export * from './netatmo'; export * from './camera';`. Consuming files should import explicitly: `import type { HueLight } from '@/types/external-apis/hue'` or `import type { HueLight } from '@/types/external-apis'`. Verify the path alias `@/types/` resolves correctly via tsconfig.json paths.
