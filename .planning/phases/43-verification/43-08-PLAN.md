...
- **STOP HERE** — Do NOT run `npm run build` without explicit user approval (see checkpoint task below). The project rule says "NEVER execute npm run build". The user's Phase 43 context OVERRIDES this rule for a ONE-TIME final verification, but the executor MUST get user confirmation before running it.
...
<task type="checkpoint:human-approve" gate="blocking">
  <what-asking>
**One-time exception to project rule:** The project CLAUDE.md says "NEVER execute npm run build". However, Phase 43 CONTEXT.md requires build verification (VERIFY-01). This checkpoint asks: May I run `npm run build` ONCE to verify the TypeScript migration?

If approved:
- Run `npm run build` (one-time only)
- Run dev server smoke test (curl key pages)
- Report results

If denied:
- Skip build verification (VERIFY-01 marked as user-skipped)
- Still complete all other verification (tsc, tests, config, allowJs)
  </what-asking>
  <resume-signal>Type "approved" to allow one-time npm run build, or "skip" to skip build verification.</resume-signal>
</task>

<task type="auto">
  <name>Task 2: Run npm run build and dev server smoke test (only if checkpoint approved)</name>
  <files></files>
  <action>
**Only execute if checkpoint above was approved.**
...
- Dev server started and all 4 pages returned 200 status
  </verify>
  <done>
VERIFY-01 and VERIFY-04 confirmed:
...
- npm run build completes successfully (if approved above)
