...
<task type="auto">
  <name>Task 1: Create Playwright config and test context</name>
  <files>
    playwright.config.ts
    tests/helpers/test-context.ts
  </files>
  <action>
Create the Playwright configuration and shared test context:
...
- The `storageState` approach handles httpOnly cookies, localStorage, and sessionStorage automatically - no manual cookie management needed.
  ...
  - Config has `webServer` with `npm run dev` and `reuseExistingServer: !process.env.CI`
  </verify>
  <done>
  - playwright.config.ts defines setup+chromium projects with storageState caching
  - Test context provides environment-sourced credentials and path constants
  </done>
</task>

<task type="auto">
  <name>Task 2: Create auth helpers and auth setup with session caching</name>
  <files>
    tests/helpers/auth.helpers.ts
    tests/auth.setup.ts
  </files>
  <action>
Create auth helpers and the Playwright setup file that authenticates once and caches the session:

**1. `tests/helpers/auth.helpers.ts`**:
...
- The `storageState` approach handles httpOnly cookies, localStorage, and sessionStorage automatically - no manual cookie management needed.
  ...
  - `cat tests/auth.setup.ts` shows signIn + storageState save to AUTH_FILE path
...
<task type="auto">
  <name>Task 3: Update .gitignore for auth session caching</name>
  <files>
    .gitignore
  </files>
  <action>
