...
must_haves:
  truths:
    ...
    - "All feature tests run with cached storageState (no per-test login)"
    - "Tests use role-based and test-id selectors (no CSS class selectors)"
  artifacts:
...
**IMPORTANT CONTEXT:** Before writing selectors, READ the actual component source files to discover the real DOM structure, test IDs, button labels, and text content. The research examples used hypothetical selectors. The executor MUST read:
- `app/components/devices/stove/StoveCard.tsx` - for stove button labels, status text, test IDs
- `app/components/devices/thermostat/ThermostatCard.tsx` - for thermostat UI structure, schedule controls
