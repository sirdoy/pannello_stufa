...
must_haves:
  ...
  key_links:
    - from: ".github/workflows/playwright.yml"
      ...
      via: "npx playwright test reads config; config uses CI-conditional webServer command"
      pattern: "npx playwright test"
    - from: "playwright.config.ts"
      to: ".github/workflows/playwright.yml"
      via: "webServer command uses process.env.CI to switch between npm run start (CI) and npm run dev (local)"
      pattern: "process\\.env\\.CI.*npm run start"
    - from: ".github/workflows/playwright.yml"
      to: "GitHub Secrets"
...
<task type="auto">
  <name>Task 2: Update Playwright config for CI and create GitHub Actions workflow</name>
  <files>
    playwright.config.ts
    .github/workflows/playwright.yml
  </files>
...
**STEP 2: Update `playwright.config.ts` webServer for CI:**

Before creating the workflow file, modify `playwright.config.ts` (created in plan 51-01) to use `npm run start` in CI (serving the built app) and `npm run dev` locally:
...
This is required because the CI workflow runs `npm run build` first, then `npx playwright test` triggers `npm run start` (which serves the pre-built app). Locally, `npm run dev` is used for faster iteration without a build step.

**IMPLEMENTATION NOTES:**
1. **Build before test in CI**: The workflow runs `npm run build` to create the production build, then Playwright's `webServer` config starts `npm run start` to serve it. This is faster and more stable than `npm run dev` in CI.
...
7. **Firebase env vars**: Some may not be available as GitHub secrets yet. Use fallback values for build step to prevent build failures. At runtime, the tests will need real Firebase config to interact with the app.
  ...
  <verify>
  - `grep -A3 'webServer' playwright.config.ts` shows `process.env.CI ? 'npm run start' : 'npm run dev'`
  - `cat .github/workflows/playwright.yml` shows workflow with PR and push triggers
  - Workflow installs chromium, builds app, runs playwright test, uploads artifacts
  ...
  <done>
  - playwright.config.ts uses `npm run start` in CI and `npm run dev` locally
  - GitHub Actions workflow runs Playwright E2E tests on every PR to main
  - CI builds app, installs Chromium browser, runs tests with encrypted secrets
