...
Purpose: Close the second major gap — stove/page.tsx duplicates 100% of StoveCard's state management and commands. By reusing existing hooks and extracting only the page-specific layout, we eliminate code duplication and achieve the orchestrator pattern.
...
<task type="auto">
  ...
  <action>
Create the page-specific utilities and presentational components that stove/page.tsx needs. These are layout-specific — they don't exist in StoveCard because StoveCard uses a card layout while stove/page.tsx uses a full-page immersive layout.
...
4. `wc -l app/stove/stovePageTheme.ts app/stove/components/*.tsx` — all files < 300 LOC
  </verify>
  <done>
...
NOTE: stove/page.tsx needs `useState` for toast state and `useCallback` for offline wrappers — that is acceptable since the page adds page-specific behavior on top of shared hooks. This is different from LightsCard which has zero extra state.
...
**However**, reviewing useStoveData more carefully — it exposes `isOnline` but NOT `queueStoveCommand`. The `useBackgroundSync` hook is called inside useStoveData but only `hasPendingCommands`, `pendingCommands`, and `lastSyncedCommand` are exposed.

**Solution:** Import `useBackgroundSync` directly in the page orchestrator for `queueStoveCommand`. This is acceptable — the page adds offline-queue behavior that the card doesn't have:
...
**Step 6: Fan/Power wrappers to adapt number → event interface.**
```typescript
...
3. `npx jest --testPathPattern="StovePage" --no-coverage` — all tests pass
...
<verification>
1. `wc -l app/stove/page.tsx` — target < 250 LOC
2. `wc -l app/stove/components/*.tsx app/stove/stovePageTheme.ts` — each < 300 LOC
3. `npx tsc --noEmit` — no TypeScript errors
4. `npx jest --testPathPattern="stove|Stove" --no-coverage` — all stove tests pass
