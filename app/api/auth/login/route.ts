 ...
 * (see lib/auth/sessionCookie.ts for the analogous flag set) so the client can distinguish
 ...
 *   - T-170-09: secure flag in production only (matches lib/auth/sessionCookie.ts).
 *
...
export const POST = withAuthAndErrorHandler(async (request?: Request) => {
  ...
  // Set the httpOnly session-marker cookie (D-15, flags mirror lib/auth/sessionCookie.ts).
