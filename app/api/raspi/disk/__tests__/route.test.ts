...
describe('GET /api/raspi/disk', () => {
  ...
  it('should return 200 with disk data', async () => {
    const mockData = {
    ...
    expect(data.percent).toBe(51.5);
    expect(mockRaspiClient.getDisk).toHaveBeenCalled();
  });

  it('should propagate ApiError from raspiClient', async () => {
    mockRaspiClient.getDisk.mockRejectedValue(
