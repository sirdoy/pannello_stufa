/**
 * Tests for GET /api/raspi/health
 */
...
describe('GET /api/raspi/health', () => {
  let mockRequest: Request;

  beforeEach(() => {
    jest.clearAllMocks();
    mockRequest = new Request('http://localhost:3000/api/raspi/health');
  ...
  it('should return 200 with health data', async () => {
    const mockData = { status: 'ok' as const, data_freshness: 'LIVE' as const };
    mockRaspiClient.getHealth.mockResolvedValue(mockData);
    ...
    expect(mockRaspiClient.getHealth).toHaveBeenCalled();
  });

  it('should propagate ApiError from raspiClient', async () => {
    mockRaspiClient.getHealth.mockRejectedValue(
