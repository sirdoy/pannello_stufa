/**
 * Tests for Fritz!Box History Bandwidth Hourly Route
...
describe('GET /api/fritzbox/history/bandwidth/hourly', () => {
  let mockRequest: Request;
  const mockSession = { user: { sub: 'auth0|123', email: 'test@test.com' } };
  const mockData = {
    items: [
      {
        hour_timestamp: 1711324800,
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    mockRequest = new Request('http://localhost:3000/api/fritzbox/history/bandwidth/hourly');
  ...
  it('should return 200 with hourly data', async () => {
    ...
    expect(data).toEqual({ success: true, hourly: mockData });
    expect(mockCheckRateLimit).toHaveBeenCalledWith('auth0|123', 'history-bandwidth-hourly');
  });
  ...
  it('should propagate errors', async () => {
    ...
    expect(data.success).toBe(false);
    expect(data.error).toBe('Hourly bandwidth query failed');
  });
});
