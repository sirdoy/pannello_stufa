/**
 * Tests for POST /api/v1/sonos/zones/[groupId]/next
 */
...
describe('POST /api/v1/sonos/zones/[groupId]/next', () => {
  let mockRequest: Request;
  let mockContext: { params: Promise<{ groupId: string }> };

  beforeEach(() => {
    jest.clearAllMocks();
    mockRequest = new Request('http://localhost:3000/api/v1/sonos/zones/RINCON_123/next', {
      method: 'POST',
    });
  ...
  it('should return 202 with command response', async () => {
    mockNext.mockResolvedValue({ success: true } as any);
    ...
    expect(mockNext).toHaveBeenCalledWith('RINCON_123');
  });
});
