/**
 * Tests for POST /api/v1/sonos/zones/[groupId]/pause
 */
...
const mockPause = jest.mocked(sonosProxy.pause);
const mockSession = { user: { sub: 'auth0|123', email: 'test@test.com' } };

describe('POST /api/v1/sonos/zones/[groupId]/pause', () => {
  let mockRequest: Request;
  let mockContext: { params: Promise<{ groupId: string }> };

  beforeEach(() => {
    jest.clearAllMocks();
    mockRequest = new Request('http://localhost:3000/api/v1/sonos/zones/RINCON_123/pause', {
      method: 'POST',
    });
  ...
  it('should return 202 with command response', async () => {
    mockPause.mockResolvedValue({ success: true } as any);
    ...
    expect(mockPause).toHaveBeenCalledWith('RINCON_123');
  });
});
