/**
 * Tests for POST /api/v1/sonos/zones/[groupId]/stop
 */
...
const mockStop = jest.mocked(sonosProxy.stop);
const mockSession = { user: { sub: 'auth0|123', email: 'test@test.com' } };

describe('POST /api/v1/sonos/zones/[groupId]/stop', () => {
  let mockRequest: Request;
  let mockContext: { params: Promise<{ groupId: string }> };

  beforeEach(() => {
    jest.clearAllMocks();
    mockRequest = new Request('http://localhost:3000/api/v1/sonos/zones/RINCON_123/stop', {
      method: 'POST',
    });
  ...
  it('should return 202 with command response', async () => {
    mockStop.mockResolvedValue({ success: true } as any);
    ...
    expect(mockStop).toHaveBeenCalledWith('RINCON_123');
  });
});
