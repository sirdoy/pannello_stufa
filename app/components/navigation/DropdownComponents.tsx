...
export function DropdownContainer({ children, className = '', align = 'left' }: DropdownContainerProps) {
  ...
  return (
    <div className={`absolute ${alignmentClass} mt-3 bg-slate-900/[0.95] ] backdrop-blur-[80px] border border-white/15 rounded-2xl shadow-liquid-xl overflow-hidden z-[9000] ring-1 ring-white/10 ring-inset animate-dropdown ${className}`}>
      <div className="p-1.5">
        {children}
      </div>
    </div>
...
export function DropdownItem({
  ...
  const activeClasses = 'bg-ember-500/30 text-ember-300 shadow-liquid-sm';
  const inactiveClasses = 'text-slate-200 hover:bg-slate-800/80 hover:shadow-liquid-sm hover:scale-[1.02]';
...
export function DropdownInfoCard({ title, subtitle, details, className = '' }: DropdownInfoCardProps) {
  return (
    <div className={`px-4 py-3.5 mb-1 rounded-xl bg-slate-800/60 ${className}`}>
...
export function MenuSection({ icon, title, children, hasBorder = false, className = '' }: MenuSectionProps) {
  return (
    <div className={`space-y-2 ${hasBorder ? 'pt-4 mt-4 border-t border-white/15 ' : ''} ${className}`}>
      {title && (
...
export function MenuItem({
  ...
  const activeClasses = 'bg-ember-500/30 text-ember-300 shadow-liquid-md ring-1 ring-ember-500/40 ring-inset';
  const inactiveClasses = variant === 'prominent'
    ? 'text-white bg-gradient-to-br from-ember-600 to-flame-700 hover:from-ember-700 hover:to-flame-800 shadow-liquid-lg hover:shadow-liquid-xl ring-1 ring-ember-600/50 ring-inset'
    : 'text-slate-200 bg-slate-800/60 hover:bg-slate-800/90 shadow-liquid-sm ring-1 ring-white/15 ring-inset';
...
export function UserInfoCard({ icon: Icon, name, email, className = '' }: UserInfoCardProps) {
  return (
    <div className={`flex items-center gap-3 px-4 py-4 rounded-2xl bg-slate-800/70 backdrop-blur-2xl shadow-liquid-md ring-1 ring-white/15 ring-inset ${className}`}>
      <div className="flex items-center justify-center w-12 h-12 rounded-xl bg-gradient-to-br from-ember-500/30 to-flame-500/30 ">
        {Icon && <Icon className="w-6 h-6 text-ember-300 " />}
      </div>
      <div className="flex-1 min-w-0">
        <Text variant="body" size="sm" className="truncate">
          {name}
