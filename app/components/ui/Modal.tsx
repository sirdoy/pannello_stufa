...
const ModalContent = forwardRef<React.ElementRef<typeof DialogPrimitive.Content>, ModalContentProps>(
...
export interface ModalProps extends Omit<ModalContentProps, 'children'> {
  ...
  maxWidth?: string; // Legacy prop
  closeOnOverlayClick?: boolean; // Consumed but not used (Radix default behavior)
  closeOnEscape?: boolean; // Consumed but not used (Radix default behavior)
}

function Modal({ isOpen, onClose, children, size = 'md', maxWidth, className, closeOnOverlayClick, closeOnEscape, ...props }: ModalProps) {
...
(Modal as ModalComponent).Description = ModalDescription;
(Modal as ModalComponent).Footer = ModalFooter;
(Modal as ModalComponent).Close = ModalClose;
