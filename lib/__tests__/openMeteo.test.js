...
describe('openMeteo', () => {
  ...
  describe('fetchWeatherForecast', () => {
    it('constructs correct API URL with all required parameters', async () => {
      ...
      // Verify hourly parameters (extended for hourly forecast)
      expect(calledUrl).toMatch(/hourly=.*temperature_2m/);
      expect(calledUrl).toMatch(/hourly=.*weather_code/);
      expect(calledUrl).toMatch(/hourly=.*precipitation_probability/);
      expect(calledUrl).toMatch(/hourly=.*wind_speed_10m/);

      // Verify no past_hours or forecast_hours (removed for full day data)
      expect(calledUrl).not.toContain('past_hours');
