  ...
  if (stoveOn) {
    ...
    if (accessToken) {
      ...
      if (intentResult.manualChange) {
        ...
        await sendCoordinationNotification(userId, 'automation_paused', {
    ...
    const callback = async () => {
      if (stoveOn) {
        ...
        if (result.success) {
          ...
          // Send notification (if throttle allows)
          await sendCoordinationNotification(userId, 'coordination_applied', {
            appliedRooms: result.appliedRooms,
          });

          // Send additional notification for capped rooms
          if (result.cappedRooms.length > 0) {
            await sendCoordinationNotification(userId, 'max_setpoint_reached', {
        ...
        if (result.success) {
          ...
          // Send notification (if throttle allows)
          await sendCoordinationNotification(userId, 'coordination_restored', {
