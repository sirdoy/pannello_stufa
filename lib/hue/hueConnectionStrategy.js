...
export class HueConnectionStrategy {
  ...
  static async getProvider() {
    try {
      ...
      const firebasePath = getEnvironmentPath('hue');
      console.log('🔍 [Hue Strategy] Environment:', {
      ...
      console.log('🔍 [Hue Strategy] Local connection:', {
        hasBridgeIp: !!localConnection?.bridgeIp,
        hasUsername: !!localConnection?.username,
        bridgeIp: localConnection?.bridgeIp || null,
      });
      ...
      if (localConnection?.bridgeIp && localConnection?.username) {
        ...
        console.log('🔍 [Hue Strategy] Local bridge reachable:', isLocalReachable);
      ...
      const hasRemote = await hasRemoteTokens();
      console.log('🔍 [Hue Strategy] Has remote tokens:', hasRemote);

      if (hasRemote) {
        console.log('⚠️ Local bridge not reachable, trying Hue Remote API...');

        const username = await getUsername();
        console.log('🔑 [Hue Strategy] Username:', username ? `${username.substring(0, 8)}...` : '❌ missing');
        if (!username) {
        ...
        console.log('🔐 [Hue Strategy] Token result:', { hasToken: !!accessToken, error, message });
