...
async function checkLocalBridge(bridgeIp, timeout = 2000) {
  ...
  try {
    ...
    const response = await fetch(`https://${bridgeIp}/clip/v2/resource/bridge`, {
      signal: controller.signal,
      // Bridge uses self-signed certificate
      // Note: fetch doesn't support rejectUnauthorized in Node.js, but works in browser
    });

    clearTimeout(timeoutId);
    return response.ok || response.status === 403; // 403 = bridge responding but unauthorized (still reachable)
  } catch (error) {
    // Timeout, network error, or CORS error = not reachable
    return false;
  }
}
