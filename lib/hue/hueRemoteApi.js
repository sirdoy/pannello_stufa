...
class HueRemoteApi {
  constructor(username, accessToken) {
    ...
    console.log('🔧 [Hue Remote API] Initialized with baseUrl:', this.baseUrl);
  }
  ...
  async request(endpoint, options = {}, isRetry = false) {
    ...
    console.log(`🌐 [Hue Remote API] ${options.method || 'GET'} ${url}`);
  ...
  async getRooms() {
    ...
    const groupTypes = Object.entries(response).map(([id, g]) => ({ id, type: g.type, name: g.name }));
    console.log('🔍 [Hue Remote API] Groups found:', groupTypes.length, groupTypes);
    ...
    console.log('🔍 [Hue Remote API] Rooms after filter:', rooms.length);
    return { data: rooms };
  }
