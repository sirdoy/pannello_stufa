...
export async function trackUsageHours(stoveStatus) {
  try {
    ...
    const newCurrentHours = data.currentHours + hoursToAdd;
    const newPercentage = (newCurrentHours / data.targetHours) * 100;
    ...
    // Check if notification should be sent (data will be returned to caller)
    const notificationData = shouldSendMaintenanceNotification(
      newPercentage,
      newCurrentHours,
      data.targetHours,
      data.lastNotificationLevel
    );

    // If notification should be sent, update lastNotificationLevel
    if (notificationData) {
      await update(maintenanceRef, {
        lastNotificationLevel: notificationData.notificationLevel,
      });
    }
...
export async function confirmCleaning(user) {
  try {
    ...
    await update(maintenanceRef, {
      ...
      lastNotificationLevel: 0  // Reset notification tracking for next cycle
    });
