    ...
    if (schedule.zones && Array.isArray(schedule.zones)) {
      parsed.zones = schedule.zones.map(zone => {
        ...
        if (zone.temp !== undefined) {
          ...
          // Multi-room: use first room's temperature as representative
          zoneData.temp = zone.rooms_temp[0].temp;
        } else if (zone.rooms && Array.isArray(zone.rooms) && zone.rooms.length > 0 && zone.rooms[0].therm_setpoint_temperature !== undefined) {
          // Fallback: use therm_setpoint_temperature from first room
          zoneData.temp = zone.rooms[0].therm_setpoint_temperature;
        }
