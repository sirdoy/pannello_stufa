...
export async function enforceStoveSyncSetpoints(stoveIsOn) {
  ...
  if (stoveIsOn && config.stoveMode) {
    console.log(`🔍 Stove sync enforcement: verifying setpoints for ${config.rooms.length} room(s)`);
    ...
    const homeStatus = await NETATMO_API.getHomeStatus(accessToken, homeId);
    console.log(`🔍 Netatmo returned ${homeStatus.rooms?.length || 0} rooms in status`);
    ...
    for (const room of config.rooms) {
      ...
      if (currentSetpoint === undefined || currentSetpoint === null) {
        console.log(`⚠️ Room "${room.name}" setpoint unknown, will re-apply stove temperature`);
      ...
      if (Math.abs(currentSetpoint - config.stoveTemperature) > 0.5) {
        console.log(`⚠️ Room "${room.name}" has setpoint ${currentSetpoint}°C, expected ${config.stoveTemperature}°C`);
        roomsNeedingFix.push({
          ...room,
          currentSetpoint,
          expectedSetpoint: config.stoveTemperature,
        ...
        console.log(`✓ Room "${room.name}" setpoint OK: ${currentSetpoint}°C`);
      }
    }
    ...
    if (roomsNeedingFix.length > 0) {
      ...
      for (const room of roomsNeedingFix) {
        try {
          ...
          if (success) {
            fixedCount++;
            console.log(`✅ Room "${room.name}" re-enforced: ${room.currentSetpoint}°C → ${config.stoveTemperature}°C`);
          } else {
