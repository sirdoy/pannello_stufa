...
export async function getValidAccessToken() {
  try {
    ...
    const cachedTokenSnap = await get(ref(db, getEnvironmentPath('netatmo/access_token_cache')));
    const cachedData = cachedTokenSnap.exists() ? cachedTokenSnap.val() : null;
...
async function doTokenRefresh() {
  ...
  if (data.error) {
    ...
    if (data.error === 'invalid_grant' || data.error === 'invalid_token') {
      ...
      await set(ref(db, getEnvironmentPath('netatmo/access_token_cache')), null);
      return {
        accessToken: null,
        error: 'TOKEN_EXPIRED',
  ...
  await set(ref(db, getEnvironmentPath('netatmo/access_token_cache')), {
