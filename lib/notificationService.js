...
export async function getFCMToken(userId) {
  ...
  await debugLog('getFCMToken chiamato', {
    ...
    VAPID_KEY_exists: !!VAPID_KEY,
    VAPID_KEY_length: VAPID_KEY?.length || 0,
    VAPID_KEY_preview: VAPID_KEY ? `${VAPID_KEY.substring(0, 10)}...` : 'undefined',
    env_check: {
  ...
  try {
    ...
    // Ottieni token FCM
    await debugLog('Getting FCM token...', { vapidKeyLength: VAPID_KEY.length });
